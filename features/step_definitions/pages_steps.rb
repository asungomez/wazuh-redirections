...
Given("The branch has a renamed page") do
  @previous_branch = FactoryBot.create(:branch, version: '2.1')
  @branch = FactoryBot.create(:branch, version: '2.2')
...
When("I mark the renamed page as new") do
  visit renamed_pages_branch_path(@branch)
  page.find('tr', text: @renamed_page_new.path).click_on 'Edit'
  select 'New page', from: 'redirection_type'
  click_on 'Save'
end
...
Then("I should not see the new page in the renamed pages list") do
  visit renamed_pages_branch_path(@branch)
  expect(page).not_to have_content(@renamed_page_new.path)
end

Then("I should not see the old page in the renamed pages list") do
  visit renamed_pages_branch_path(@branch)
  expect(page).not_to have_content(@renamed_page_deleted.path)
end
...
Then("I should see the old page in the deleted pages list") do
  visit deleted_pages_branch_path(@branch)
  expect(page).to have_content(@renamed_page_deleted.path)
end
...
Then("I should see the renamed page in the new pages list") do
  visit new_pages_branch_path(@branch)
  expect(page).to have_content(@renamed_page_new.path)
end